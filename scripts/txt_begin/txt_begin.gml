function txt_begin() {
	...
	txt("To the Lunarians residing on the moon, life is just continued existence."),
	ptxt(),
	...
	txt("Today, I'm here to show you a collection of stories where love is everywhere!", ME.HEHE, u, u, function() {music_set(mus.glossary)}),
	...
	*/
	ptxt(),
	ntxt(0208, u, u, s_black, function() {music_set(mus.alone)}),
	
	ntxt(0070, ME.DEF, SPRITEPOS.RIGHT, s_black),
	
	//footsteps sound and crickets, fading into kotohime with daytime forest background	
	
	ptxt(),
	txt("The Little Rabbit Princess, frolicking about in the woods.", u, u, s_forest, function() {textbox_set_type(global.textnvl)}),
	...
	txt("And off in the distance, past the countless trees, there sat a little shrine in a clearing, solitary yet serene."),
...
//	txt("~~Than the one--~~ |- ", ME.YAYNOTE, u, u, function() {music_set(mus.null)}),
	...
	ntxt(0098, ME.DEF, SPRITEPOS.HIDDEN, s_black, function() {music_set(mus.null)}),
	...
	ptxt(),
	txt("Now get out, and stay away forever!", REIMU.MIFFED, SPRITEPOS.LEFT, s_shrine, function() {textbox_set_type(global.textzpix)}),
	//maybe have like continuous BOOM sound effects with each textbox
	txt("Fine, fine! Just stop hitting me with those things!", KANAUNK.AWW, SPRITEPOS.MIDRIGHT),
	ntxt(0080, KANAUNK.MAD, SPRITEPOS.HIDDEN),
	//kana slides out of frame
	
	txt("...", REIMU.DEF),
	txt("Finally! I thought she'd never leave.", REIMU.YAY),
	ntxt(0000, REIMU.DEF, SPRITEPOS.HIDDEN, s_shrine, function() {music_set(mus.chillin)}),
	ntxt(0040),
	...
	txt("Once she was far away enough to escape the shrine maiden's wrath, she stopped moving and tried scraping off the dirt and amulets."),
	...
	txt("...And the \"trash\" is that girl that passed me?", ME.NEUTRAL),
	...
	txt("She showed up here a few weeks ago and acted like she owned the place! Flying around the shrine, playing with all my stuff, making as much noise as she could!", REIMU.MIFFED),
	...
	txt("Nope!", ME.WINKY),
	txt("...Sweep the shrine for me?", REIMU.HAHA),
	txt("Nope!", ME.WINKY),
	...
	ntxt(020, u, u, u, function() {music_set(mus.summerhill2)}),
	...
	txt("And aren't you that weird girl from the danmaku competition? The really weak one who went down in ten seconds?", KANAHAT.COCKY),	
	...
	ptxt(),
	txt("And just like that, we were friends.", u, u, s_stars, function() {textbox_set_type(global.textnvl)}),
	txt("I quickly brought Kana to my home, and went through the necessary rituals to bind her to it."),
	txt("Poltergeists like Kana are normally bound to one place of residence, you see. They cannot leave their home for more than three days, or else they will cease to be."),
	ptxt(),
	...
	txt("It was to the point that she knew more about the outside world than The Land of Fantasy."),
	...
	ptxt(),
	txt("One evening, we took an especially late stroll through the Bamboo Forest of the Lost.", u, u, s_crescent2, function() {textbox_set_type(global.textnvl)}),
	txt("It was already dark, and the moonlight was just barely stopping us from walking right into the bamboo trees. Not to mention, the more nasty fairies were just beginning to come out around this time."),
	...
	txt("The people I knew resided in a place completely isolated from most of The Land of Fantasy."),
	...
	txt("Tewi! What are you doing here?", REISEN.DEF, SPRITEPOS.CENTER),
	txt("Aren't you supposed to be fetching the laundry?", REISEN.DEF),
	...
	ntxt(40, u, u, u, function() {audio_play_sound(snd_damage7, 0, false)}),
	...
	txt("...It's not working!", REISEN.WOAH, SPRITEPOS.LEFT),
	
	
	txt("Fufufu...", KANAUNK.DEF, SPRITEPOS.HIDDEN),
	
	ntxt(0000, REISEN.DEF, SPRITEPOS.LEFT),
	//confuzzled might not fit here. dunno
	txt("Little inaba, you have much to learn.", KAGUYA.FUFUFU, SPRITEPOS.CENTER),
	...
	ntxt(0199, u, u, s_eienteihall1, function() {audio_play_sound(snd_sliding_door_C, 0, false)}),
	...
	txt("\(...I don't think so. She's lacking that special something.\)", KANAHAT.NEUTRAL),
	...
	txt("Even better if she had a huge extravagant home, long shining hair, and a slick sense of fashion!", KANAHAT.YAY),
	...
	txt("An amusing one, she is. She used to work at the Lunar Capital with the other moon rabbits, but she deserted during the Lunar War.", KAGUYA.DEF),
	...
	txt("START!!", ME.DEF, u, u, function() {music_set(mus.chesshogi)}),
	...
	txt("...#...#...#...", u, u, s_27),	
	ptxt(),
	txt("Reisen, you played Shogi with the other moon rabbits all the time, didn't you? Surely you should be better than this.", KAGUYA.DEF),
	...
	txt("THIS!", REISEN.DEF),
	ntxt(0064, u, u, s_54_1),
	ntxt(0064, u, u, s_54_2),
	ntxt(0064, u, u, s_54_3),
	ntxt(0064, u, u, s_54_4),
	ntxt(0200, u, u, s_54_5),
	
	ntxt(0000, KANAHAT.NEUTRAL, SPRITEPOS.LEFT, u, function() {music_set(mus.eientei)}),
	ntxt(0000, REISEN.SMUG, SPRITEPOS.CENTER, s_eienteihall2, function() {textbox_set_type(global.textzpix)}),
	...
	ntxt(00, KANAHAT.DEF, SPRITEPOS.HIDDEN, u, function() {music_set(mus.null)}),
...
//	txt("~~But our lives are full~~#~~Of those who make us smile as we play games and meet~~", KANAHAT.DEF),
	//wow this is
	//really awkward
...
//	txt("~~Forever is only a moment too~~", KANAHAT.DEF),
	...
	ntxt(0776, u, u, u, function() {music_set(mus.sing)}),
	...
	txt("Sorry, there's not much here. I live a simple life, you see.", MOKOU.DEF, u, u, function() {textbox_set_type(global.textzpix)}),	
	...
	txt("Sorry, it's probably stupid.", KANAHAT.SADSMILE),
	...
	ntxt(222, MOKOU.NEUTRAL, SPRITEPOS.RIGHT, u, function() {audio_play_sound(snd_woman_coughing3, 0, false)}),
	...
	ntxt(033, u, u, u, function() {music_set(mus.null)}),
	ntxt(0123, MOKOU.NEUTRAL, SPRITEPOS.RIGHT, s_mkinnt, function() {audio_play_sound(snd_game_explosion4, 0, false)}),
	//big sound effect of magic RESSURECTION BWAAHM
	//game_explosion4
	
	txt("Holy macaroni, that hurt.", MOKOU.SADSMILE, u, u, function() {music_set(mus.mokou)}),
...
//	txt("No worries, I'm all fine now.", MOKOU.YAY),
	...
	txt("...By the way, keep the whole immortality thing between us, alright? You saw it, but that ain't supposed to be a public thing.", MOKOU.DEF),
	txt("Right!", ME.YAY),
	...
	txt("Gee golly gosh gloriosky, I could never have guessed!", KANAHAT.WINKY),
	...
	txt("Mokou seemed friendly with both of us, and Kana had a little twinkle in her eye as they talked."),
	...
	txt("I had horrible red spots all over me, disfigured legs, and empty eyes.", KANAHAT.DEF),
	...
	txt("The mansion was totally empty for a while, until a new family moved in. And when they got there... just like my mother, they were terrified of me. But they left before I could befriend them.", KANAHAT.DEF),
	...
	txt("By instinct, you look for some way to get that nourishment, in any way.", MOKOU.DEF),
	...
	txt("...Or, more like you *are* something wrong.", MOKOU.DEF),
	
	ptxt(),
	txt("Then you either wither away, believing they're right... or you do something stupid.", MOKOU.DEF),
	//kinda awkwardly phrased
	
	txt("Even if you leave that place, bask in the sunlight, and eat the most delicious food in the world...", MOKOU.DEF),
	txt("Those days stick with you.", MOKOU.DEF),	
	ptxt(),
	...
	ptxt(),	
	txt("When we got there, it was far too silent. There should've been birds singing or fairies playing, but only the flow of water welcomed us.", u, u, s_mistylakesunrise, function() {music_set(mus.vanishing)}),
	...
	txt("You're my dream girl.", KANAHAT.DEF, u, u, function() {textbox_set_type(global.textzpix)}),
	//ha ha super cheesy and dumb
	//keep it or dont
	txt("And I wouldn't have it any other way.", KANAHAT.DEF),
	txt("So...", KANAHAT.DEF),
	...
	txt("The next couple days were silent too. We didn't do much together, and each time I looked into Kana's eyes, they seemed farther and farther away.", u, u, s_kotohimehall),
	...
	txt("The bonsai tree? That won't ever grow, will it? Due to your spell and all.", ME.NEUTRAL),
	txt("Oh, it's just an old habit of mine. It gives me something to do every day. Even a task so small alleviates much of my boredom.", KAGUYA.FUFUFU),
...
//	ntxt(0333, u, u, u, function() {audio_play_sound(snd_running_in_fall_forest, 0, false)}),
	...
	ntxt(0140, ME.TINYSMIRK, SPRITEPOS.LEFT),
	ntxt(080, ME.TINYSMIRK, SPRITEPOS.HIDDEN),
	ntxt(0099, u, u, s_black),
	
	txt("#END OF FORMER PART"),

	...
	if ncm(10) {
		var guy = c_makeshooter(global.gw/2+global.gw/2*-itneg(), ((count-1)%500)/5+40, 8, global.enemies.redfairy, new pattern(function() {
			...
			if count > 50 && count < 220 {
				...
				if ncm(170) {
					myguy = rmrng(360);
					c_shootfan(x, y, 1, myguy, myguy+10, 3, 3, global.bullets.point);
				}
			}
		}), 4);
		guy.reverse = itneg();
...
new cell(ROOMTYPE.TIME, function() {
	c
	if ncm(24) {
		var fary = c_makeshooter(-30, 100, 8, global.enemies.redfairy, new pattern(function() {
			c
			if ncm(50) {
...
new cell(ROOMTYPE.TIME, function() {
	...
	if ncm(16) {
		if global.me.hp > 2 {
			var fary = c_makeshooter(WIDTH/4, -20, 8, en.fairy, new pattern(c_null, function() {c_meleeshot()}));
			var farydir = ineg(rmrng(1));
			fary.hgrav = .2;
			fary.hspd = farydir*5;
			fary.vspd = 3;
		} else {
			var fary = c_makeshooter(WIDTH/4, -20, 8, en.fairy, new pattern(function() {
...
}, DIFFICULTY.EASY, u, u, function() {
	tendency = 0;
}),
new cell(ROOMTYPE.TIME, function() {
	c
	if ncm(26) {
		var fary = c_makeshooter(WIDTH/2+(WIDTH/2)*itneg(), 80, 8, en.redfairy, new pattern(function() {
			c
			if ncm(30) {
				c_shootabs(x, y, rmrng(20)/10-1, -rmrng(10)/10-1, bul.point, c_blue, function() {
...
new cell(ROOMTYPE.TIME, function() {
	...
	var i
	for (i=0; i<2; i++) {
		var fary = c_makeshooter(40+(WIDTH-80)*i, -40, 50, en.magicfairy, new pattern(function() {
			if !iterations {
				c
				if count > 100 {
					iterations = true;
				...
				if ncm(5) {
					c_shoot(x, y, 1+abs(count-80)/25, point_me()+rmrng(20)-10, bul.point);
				}
				cmm();
				cmm();
				if count <= 0 {
...
new cell(ROOMTYPE.TIME, function() {
	...
	var fary = c_makeshooter(WIDTH/2, -40, 50, en.magicfairy, new pattern(function() {
		c
		if ncm(30) {
			c_shootfan(x, y, 1.6, point_me()-10, point_me()+10, 5, 0, bul.point);
		}
		if ncm(40) {
			c_shootfan(x, y, 3, point_me()-7, point_me()+7, 3, 0, bul.point);
			count = 0;
		}
		vspd = lerp(vspd, 0, .05);
	}), 20);
...
myrooms[2].name = "fairy shoot circle shoot fairy";
